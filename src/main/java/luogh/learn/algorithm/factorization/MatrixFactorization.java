...
public class MatrixFactorization {
  ...
  public static void main(String args[]) {
    ...
    System.out.println(
        "execute MatrixFactorization succeed,cost time:" + (endTime - startTime) / 1000.00f + "s");
  }

  private static void executeMatrixFactorization(float[][] R, float[][] P, float[][] Q, int kDims,
      long iterTimes) {
...
//                          System.out.println("P[i][k]:"+P[i][k]+" Q[k][j]:"+Q[k][j]);
      ...
      if (tempVariance < LOSS_RATE) {
        ...
        if (iterTimes % 100 == 0) {
          System.out.println(
              "final answer still not find yet, current iterateTimes is:" + (ITERATE_TIMES
                  - iterTimes) + " and current" +
  ...
  /**
   * the decompostionMatrix initial datas is simply make randomly 0~10
   */
