...
/**
 * Created by Kaola on 2015/12/29. This class is called Fixed-Point Interative Method which used to
 * compute one of the solution of the NonLinear Equation;
 *
 * see:《数值分析与算法》 喻文健
 *
 * this method is trying to convert the problem of the solution of a NonLinear Equation to a
 * convergence of discrete sequence problem. the convergence of the sequence is the final wanted
 * estimated value. and the discrete sequence is generated by converting the function f(x)=0 to x =
 * g(x); for example : f(x)= x^4-x-2,so the discrete sequence can build according to the function x
 * = x^4-2,so each element in the sequence is  :X[k+1] = X[k]^4-2 ,also the function can be x =
 * Math.pow(x+2,1/4). so to choose which one function ,also need be considered in this case.
 *
 * And, the reason why it`s called Fixed Point Interative Method is that if the normal solution is
 * Xm ,then, Xm=g(Xm)
 */
...
//		double solution = fpim.execute("x^4-x-2","(x+2)^(1/4)",-2,0.00000000003,0.00000000001);
   ...
   */
  public double execute(String fFunc, String gFunc, double firstX, double thresholdFunc,
      double thresholdX) throws Exception {
    JEP jep = new JEP();
