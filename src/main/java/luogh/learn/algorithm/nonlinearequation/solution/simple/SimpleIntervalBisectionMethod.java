 ...
 * This class is called Simple Interval Bisection Method which
...
public class SimpleIntervalBisectionMethod {
	...
	/**
	 *
	 * @param nonLinearEquation
	 * @param startInterval
	 * @param endInterval
	 * @return one of the solution for the Non-Linear Equation on the Interval [startInterval,endInterval]
	 */
	public double execute(String nonLinearEquation,Double startInterval,Double endInterval,Double threshold) throws Exception {
		...
		if(startResult*endResult>0)
			throw new RuntimeException("we think this Interval:["+startInterval+","+endInterval+"] does not exit at least one resolution for" +
		...
		while(endInterval - startInterval > threshold ) {
			...
			System.out.println("Iterate Time :("+iterTime+") and the Interval ["+startInterval+","+
